describe("config module", function()
	...
	describe("setup()", function()
		...
		it("disables goto keymaps by default", function()
			config.setup({})
			for i = 1, 5 do
				assert.is_nil(config.values.keymaps["goto_" .. i])
			end
		end)

		it("merges goto keymap overrides", function()
			config.setup({
				keymaps = {
					goto_1 = "<A-1>",
					goto_3 = "<A-3>",
			...
			assert.are.equal("<A-1>", config.values.keymaps.goto_1)
			assert.is_nil(config.values.keymaps.goto_2)
			assert.are.equal("<A-3>", config.values.keymaps.goto_3)
		end)
