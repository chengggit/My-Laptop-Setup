describe("terminal module", function()
	...
	describe("create() with hidden option", function()
		it("adds a terminal without opening a window", function()
			local term = terminal.create({ hidden = true })
		...
		it("does not disturb visible windows", function()
			local first = terminal.create()
			local width_before = vim.api.nvim_win_get_width(first.win)

			terminal.create({ hidden = true })

			assert.is_true(vim.api.nvim_win_is_valid(first.win))
			assert.are.equal(width_before, vim.api.nvim_win_get_width(first.win))
		end)
	end)
	...
	describe("setup_keymaps()", function()
		...
		it("sets goto keymaps when configured", function()
			config.values.keymaps.goto_2 = "<A-2>"
			...
			for _, km in ipairs(keymaps) do
				if km.desc and km.desc:match("Goto 2") then
					has_goto = true
					break
				end
			end
			assert.is_true(has_goto, "Goto 2 keymap should be set")

			config.values.keymaps.goto_2 = nil
			vim.api.nvim_buf_delete(buf, { force = true })
		end)
	end)
end)
