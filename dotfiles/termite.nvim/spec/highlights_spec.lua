describe("highlights module", function()
	...
	describe("setup()", function()
		...
		it("applies user-provided tab highlight tables", function()
			config.setup({
				highlights = {
					tab_active = { fg = "#ff9e64", bold = true },
					tab_inactive = { fg = "#565f89", bg = "NONE" },
				},
			})
			highlights.setup()

			local active = vim.api.nvim_get_hl(0, { name = "TermiteTabActive" })
			assert.are.equal(0xff9e64, active.fg)
			assert.is_true(active.bold)
			local inactive = vim.api.nvim_get_hl(0, { name = "TermiteTabInactive" })
			assert.are.equal(0x565f89, inactive.fg)
		end)
	end)
