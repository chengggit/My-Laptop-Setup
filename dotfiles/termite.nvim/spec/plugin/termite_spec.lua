...
describe("plugin/termite", function()
	...
	describe(":Termite command", function()
		...
		it(":Termite goto focuses the indexed terminal", function()
			local termite = require("termite.init")
			local terminal = require("termite.terminal")
			terminal.create()
			terminal.create()
			vim.cmd("Termite goto 1")
			assert.are.equal(state.terminals[1].win, vim.api.nvim_get_current_win())
		end)

		it(":Termite goto ignores an out-of-range index", function()
			require("termite.init")
			local terminal = require("termite.terminal")
			terminal.create()
			assert.has_no.errors(function()
				vim.cmd("Termite goto 9")
			end)
			assert.are.equal(1, #state.terminals)
		end)

		it(":Termite reflow executes without error", function()
			local terminal = require("termite.terminal")
			terminal.create()
			terminal.create()
			assert.has_no.errors(function()
				vim.cmd("Termite reflow")
			end)
	...
	describe("autocmds", function()
		...
		describe("WinEnter border highlight fix", function()
			...
			it("keeps only the outer-edge border across WinEnter", function()
				highlights.setup()
				local termite = require("termite.init")
				...
				local border = vim.api.nvim_win_get_config(state.terminals[1].win).border
				local visible = {}
				for _, part in ipairs(border) do
					local char = type(part) == "table" and part[1] or part
					if char ~= "" and char ~= " " then
						table.insert(visible, char)
					end
				end
