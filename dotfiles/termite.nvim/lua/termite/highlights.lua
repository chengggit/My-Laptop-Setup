...
M.TAB_ACTIVE = "TermiteTabActive"
M.TAB_INACTIVE = "TermiteTabInactive"
...
M.setup = function()
	...
	-- Selected tab in the maximized tab bar - defaults to TabLineSel
	vim.api.nvim_set_hl(0, M.TAB_ACTIVE, {
		link = "TabLineSel",
		default = true,
	})

	-- Unselected tabs in the maximized tab bar - defaults to TabLine
	vim.api.nvim_set_hl(0, M.TAB_INACTIVE, {
		link = "TabLine",
		default = true,
	})
	...
	local tab_active = config.values.highlights.tab_active
	local tab_inactive = config.values.highlights.tab_inactive
	...
	if type(tab_active) == "table" then
		vim.api.nvim_set_hl(0, M.TAB_ACTIVE, tab_active)
	end
	if type(tab_inactive) == "table" then
		vim.api.nvim_set_hl(0, M.TAB_INACTIVE, tab_inactive)
	end
end
