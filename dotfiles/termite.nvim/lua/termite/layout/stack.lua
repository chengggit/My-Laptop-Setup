...
-- NOTE: Every terminal gets a fresh stored config, even hidden ones, so
-- re-showing later uses correct geometry. Per-window errors are isolated
-- so one stale window never leaves the rest of the stack broken.
M.reflow = function()
	...
	for i, term in ipairs(state.terminals) do
		...
		term.config = win_config
		if term.win and vim.api.nvim_win_is_valid(term.win) then
			pcall(M.apply_config, term, win_config)
		end
	end
end
